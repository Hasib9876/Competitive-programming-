...
for iteration in range(5):
    ...
    # Look for consistent signatures
    for pos in [158, 216, 270, 325]:  # Known signature positions
        for sig in SIGNATURES:
            ...
            # Check condition
            if total_known > 0 and matches == total_known:
                print(f"  Pos {pos}, sig {sig[:2]}: matches={matches}/{total_known} - ACCEPTING")
                ...
                print(f"  Pos {pos}, sig {sig[:2]}: matches={matches}/{total_known} - partial accept")
    
    print(f"Partial key after iteration {iteration}: {[f'{b:02x}' if b is not None else 'None' for b in partial_key]}")
